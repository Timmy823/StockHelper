...
public class MemberService {
    ...
    public JSONObject CheckMemberAndCreateLogin(CreateLoginParam data) {
        ...
        if(member == null) {
            return responseCheckMemberSuccess("error", member_map, "會員帳號或密碼錯誤");
        }
        ...
        return responseCheckMemberSuccess("OK", member_map,"");
    }

    public JSONObject responseCheckMemberSuccess(String create_status, HashMap<String, String> member_map, String create_message){
        ...
        for(int i=0; i<object_keyString.length; i++) {
            member_object.put(object_keyString[i], member_map.get(member_valuesStrings[i]));
        }
        data.put("search_member_status",create_status);
        data.put("message",create_message);
        data.put("member_data",member_object);
