...
public interface MemberRespository extends JpaRepository<MemberModel, String> {
    @Query(nativeQuery = true, value = "select count(*) from members where mid = ?1")
    public long existByUUID(String uuid);

    @Query(nativeQuery = true, value = "select count(*) from members where member_account = ?1")
    public long existByAccount(String account);

    @Query(nativeQuery = true, value = "select * from members where member_account = ?1")
    public MemberModel FindByAccount(String account);

    @Query(nativeQuery = true, value = "select * from members where member_account = ?1 and member_passwd = ?2")
