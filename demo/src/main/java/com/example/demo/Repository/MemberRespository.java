...
public interface MemberRespository extends JpaRepository<MemberModel, String> {
    @Query(nativeQuery = true, value = "select count(*) from stock_members2 where mid = ?1")
    public long existByUUID(String uuid);

    @Query(nativeQuery = true, value = "select count(*) from stock_members2 where member_account = ?1")
    public long existByAccount(String account);

    @Query(nativeQuery = true, value = "select * from stock_members2 where member_account = ?1")
