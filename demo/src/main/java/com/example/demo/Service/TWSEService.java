...
public class TWSEService {
    ...
    public JSONObject getCompanyProfitablityIndex(String stock_id) {
        try {
            ...
            String[] profit_info_string = {"year", "gross_profit", "operating_profit", "operating_income", "net_income_after_taxes", "ROE", "ROA"};
            ...
            for (Element column : table) {
                ...
                //put year
                profit_info.put(profit_info_string[profit_string_index++], column_info.get(column_info_posString[0]).text());
                ...
                profit_info.put(profit_info_string[profit_string_index++], item.contains("(") ? 
                        item.split("[(]")[0].trim() : item);
